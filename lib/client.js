// sidebar-enhancement-search — Client half (browser bundle, __ModuleLoader__ contract)
...
window.__ModuleLoader__.load({ id: 'sidebar-enhancement-search', factory: (require) => {
  var module = { exports: {} };
  ...
  function dbg() {
    ...
    args.unshift('[sidebar-enhancement-search]');
  ...
  /** All leaves whose ACTIVE tab is the built-in explorer: [{inBottom}]. */
    ...
    function scan(node, inBottom) {
      ...
      if (node.kind === 'leaf') {
        for (var i = 0; i < node.tabs.length; i++) {
          if (node.tabs[i].id === node.active && node.tabs[i].type === 'explorer') {
  ...
  var MOUNT_ATTR = 'data-sidebar-enhancement-search';
  ...
  /**
  * Find the built-in explorer's header row. better-sidebar's CSS module tokens
  * keep the source class name as a suffix (e.g. `W-zNGW_explorerHeader`), so
  * `[class*="explorerHeader"]` stays stable across rebuilds. Only visible
  * elements count (hidden pane tabs have no layout), so the mount lands in the
  * explorer that is actually shown.
  */
  function locateMount() {
    var headers = Array.prototype.slice.call(document.querySelectorAll('[class*="explorerHeader"]'))
      .filter(function (h) { return h.getClientRects().length > 0; });
    if (headers.length === 0) {
      dbg('locateMount: no visible explorerHeader found');
      return null;
    }
    var header = headers[0];
    var root = header.parentElement;
    if (!root) {
      dbg('locateMount: explorerHeader has no parent');
      return null;
    }
    ...
    if (header.nextSibling) root.insertBefore(mount, header.nextSibling);
    else root.appendChild(mount);
    dbg('locateMount: search bar inserted below explorer header');
    return mount;
  }
  ...
  // ==========================================================================
  var STYLE_ID = 'sidebar-enhancement-search-badges';
  ...
  function SearchBar(props) {
    ...
    React.useEffect(function () {
      function measure() {
        ...
        setMaxH(Math.max(160, Math.floor(window.innerHeight - rect.top - 12)));
      }
      measure();
    ...
    function ensureIndex() {
      ...
      setStatus('loading');
      fetch('/sidebar-enhancement-search/index?sessionId=' + encodeURIComponent(sessionId))
    ...
    function reveal(path) {
      fetch('/sidebar-enhancement-search/reveal?sessionId=' + encodeURIComponent(sessionId) + '&path=' + encodeURIComponent(path)).catch(function () {});
    }
    ...
    if (slots) {
      var reg = slots.inject('shell.overlay', function () {
        return slots.register({
          name: 'shell.overlay',
          id: 'sidebar-enhancement-search',
          order: 4,
          label: '文件筛选',
          registrant: 'sidebar-enhancement-search'
        }, ExplorerAugment);
      });
