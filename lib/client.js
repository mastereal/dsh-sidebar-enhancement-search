...
window.__ModuleLoader__.load({ id: 'dsh-sidebar-enhancement-search', factory: (require) => {
  ...
  *  v1.0.6: better-sidebar 0.16.1 REOPENS the files window as the built-in tab
  *  `{ id: "editor", title: t("files") }` — localized "文件" in zh (only the
  *  seeded default state hardcodes the English "Files"), so match both plus
  *  any editor tab whose meta explicitly sets treeOpen (the files window's
  *  marker). */
    ...
    function scan(node, inBottom) {
      ...
      if (node.kind === 'leaf') {
        for (var i = 0; i < node.tabs.length; i++) {
          var tab = node.tabs[i];
          var meta = tab.meta;
          var isFiles = tab.type === 'explorer' ||
            (tab.type === 'editor' && (tab.title === 'Files' || tab.title === '文件')) ||
            (meta !== null && meta !== undefined && meta.treeOpen === true);
