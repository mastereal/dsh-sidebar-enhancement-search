...
function sessionCwd(ctx, sessionId) {
  ...
  if (sessionId) {
    console.log(`[dsh-sidebar-enhancement-search] sessionCwd failed for ${sessionId} (falling back to process.cwd())`);
  }
  return "";
}

/**
 * Bring the newly opened explorer window to the FOREGROUND. Explorer windows
 * spawned by a non-foreground process (DSH) open BEHIND other windows, even
 * though the system allows foreground steals (ForegroundLockTimeout=0). This
 * helper runs detached: it polls (up to ~5s) for the explorer window whose
 * title matches the opened folder's basename, fakes an Alt keypress (classic
 * foreground-unlock), then SetForegroundWindow. v1.0.4.
 */
function activateExplorerWindow(targetAbs) {
  const folder = basename(targetAbs);
  const sig =
    "using System;using System.Runtime.InteropServices;public class FG{" +
    '[DllImport("user32.dll")] public static extern void keybd_event(byte,byte,uint,System.UIntPtr);' +
    '[DllImport("user32.dll")] public static extern bool SetForegroundWindow(IntPtr);}';
  const helper =
    "$t=" + JSON.stringify(folder) + ";" +
    "$sig=" + JSON.stringify(sig) + ";" +
    "for($i=0;$i -lt 12;$i++){" +
    "$w=Get-Process explorer -ErrorAction SilentlyContinue|Where-Object{$_.MainWindowTitle -like '*'+$t+'*'}|Select-Object -First 1;" +
    "if($w){Add-Type -TypeDefinition $sig;" +
    "[FG]::keybd_event(0x12,0,0,[System.UIntPtr]::Zero);[FG]::keybd_event(0x12,0,2,[System.UIntPtr]::Zero);" +
    "[FG]::SetForegroundWindow($w.MainWindowHandle)|Out-Null;break}" +
    "Start-Sleep -Milliseconds 400}";
  try {
    const child = spawn("powershell.exe", ["-NoProfile", "-Command", helper], { detached: true, stdio: "ignore" });
    child.unref();
  } catch (e) {}
}
...
// Node's internal ESM loader returns the DEFAULT export only; 'webServer' and
// 'sessions' are hard dependencies so apply runs only after both exist.
// v1.0.5: 'sessions' was missing from inject, so ctx.sessions was undefined and
// sessionCwd() always fell back to process.cwd() (the DSH service's launch
// directory) — the search index then covered the WRONG folder whenever the
// service was started outside the session workspace.
