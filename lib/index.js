// sidebar-enhancement-search — Host half
// Serves three routes for the Explorer filter tab:
//   GET /sidebar-enhancement-search/index?cwd=     full recursive file index (cached)
//   GET /sidebar-enhancement-search/tree?cwd=&dir= one directory listing (lazy tree)
//   GET /sidebar-enhancement-search/reveal?cwd=&path= open the containing folder in the
//                                          OS file manager (explorer /select)
import { readdirSync, statSync, existsSync } from "node:fs";
import { join, resolve, sep, basename, dirname } from "node:path";
import { spawn } from "node:child_process";

export const name = "sidebar-enhancement-search";
...
function revealInExplorer(cwd, path, res) {
  ...
  try {
    ...
    child.unref();
    console.log(`[sidebar-enhancement-search] reveal ${select ? "select" : "open"} ${target}`);
...
export function apply(ctx) {
  ...
  disposers.push(
    webServer.register({
      kind: "prefix",
      path: "/sidebar-enhancement-search",
      handler: (req, res) => {
        if (!trusted(req)) {
          res.writeHead(403);
          res.end("forbidden");
        ...
        try {
          ...
          if (route === "/sidebar-enhancement-search/index") {
          ...
          if (route === "/sidebar-enhancement-search/tree") {
          ...
          if (route === "/sidebar-enhancement-search/reveal") {
